...
class FeatureCreep {
  ...
  void create(Level level, /*bool isDescending, int depth, */ options) {
    ...
    factory = new FeatureFactory(this /*, depth*/);
  ...
  void tryCreate() {
    ...
    */
    ...
    connectors.clear();
    startPos = null;
    startPos = factory.makeStartingRoom().center;
    ...
    for (var tries = 0; (tries < options.maxTries) && (connectors.length > 0); tries++) {
      ...
      switch (connector.from) {
        case ConnectFrom.ROOM:
          // Try to add a hall.
          success = factory.makeHall(connector);
          break;

        case ConnectFrom.HALL:
          final choice = rng.range(100);
          if (choice < options.chanceOfRoom) {
            success = factory.makeRoom(connector);
          } else {
            // Default to a junction.
            success = factory.makeJunction(connector);
          }
          /*
...
class FeatureFactory {
  final FeatureCreep creep;

  FeatureFactory(this.creep);
  ...
  */
  ...
  bool makeHall(Connector connector) {
    final length = rng.range(creep.options.hallLengthMin, creep.options.hallLengthMax);
    ...
    if (!creep.isOpen(bounds, null)) return false;
    ...
    if (!creep.level.bounds.contains(pos)) return false;
    if (!creep.level.bounds.contains(leftCorner)) return false;
    if (!creep.level.bounds.contains(rightCorner)) return false;

    if ((creep.getTile(leftCorner) != TileType.WALL) &&
        (creep.getTile(pos) == TileType.WALL)) return false;

    if ((creep.getTile(rightCorner) != TileType.WALL) &&
        (creep.getTile(pos) == TileType.WALL)) return false;

    // Place the hall.
    var step = connector.pos;
    for (var i = 0; i <= length; i++) {
      creep.setTile(step, TileType.FLOOR);
    ...
    */
    ...
    creep.addHallConnector(connector.pos + (connector.direction * length),
        connector.direction);
  ...
  bool makeJunction(Connector connector) {
    ...
    if ((choice -= creep.options.chanceOfTurn) < 0) {
      if (rng.oneIn(2)) left = true; else right = true;
    } else if ((choice -= creep.options.chanceOfFork) < 0) {
      if (rng.oneIn(2)) left = true; else right = true;
      straight = true;
    } else if ((choice -= creep.options.chanceOfTee) < 0) {
      left = true;
      right = true;
    } else if ((choice -= creep.options.chanceOfFourWay) < 0) {
      left = true;
      right = true;
      straight = true;
    } else {
    ...
    if (!creep.isOpen(rect, center + connector.direction.rotate180)) return false;

    // Place the junction.
    creep.setTile(center, TileType.FLOOR);

    // Add the connectors.
    if (left) creep.addRoomConnector(center + connector.direction.rotateLeft90, connector.direction.rotateLeft90);
    if (right) creep.addRoomConnector(center + connector.direction.rotateRight90, connector.direction.rotateRight90);
    if (straight) creep.addRoomConnector(center + connector.direction, connector.direction);
  ...
  private bool MakeStair(Connector connector)
  {
    ...
    if (!creep.IsOpen(rect, connector.pos + connector.Direction.Rotate180)) return false;

    TileType type = (Rng.Int(10) < 6) ? TileType.StairsDown : TileType.StairsUp;
    creep.SetTile(connector.pos, type);
  ...
  private bool MakeMaze(Connector connector)
  {
    ...
    int width = Rng.Int(creep.Options.MazeSizeMin, creep.Options.MazeSizeMax);
    int height = Rng.Int(creep.Options.MazeSizeMin, creep.Options.MazeSizeMax);
    ...
    {
      creep.SetTile(pos, TileType.Floor);
    }
    ...
    if (Rng.OneIn(2))
    {
      foreach (Vec pos in bounds.Inflate(-1))
      {
        creep.SetTile(pos, TileType.LowWall);
      }
    }
    ...
    maze.Draw(pos => creep.SetTile(pos + offset, TileType.Floor));

    creep.LightRect(bounds, mDepth);
  ...
  private bool MakePit(Connector connector)
  {
    ...
    int width = Rng.Int(creep.Options.RoomSizeMin, creep.Options.RoomSizeMax);
    int height = Rng.Int(creep.Options.RoomSizeMin, creep.Options.RoomSizeMax);
    ...
    // choose a group
    IList<Race> races = creep.Content.Races.AllInGroup(Rng.Item(creep.Content.Races.Groups));
    ...
    RoomDecoration.DecorateInnerRoom(bounds, new RoomDecorator(this,
      pos => creep.AddEntity(new Monster(pos, Rng.Item(races)))));
  ...
  */
  ...
  Rect createRoom(Connector connector) {
    final width = rng.range(creep.options.roomSizeMin, creep.options.roomSizeMax);
    final height = rng.range(creep.options.roomSizeMin, creep.options.roomSizeMax);
    ...
    RoomDecoration.Decorate(bounds, new FeatureFactory.RoomDecorator(this,
      pos => creep.Populate(pos, 60, 200, mDepth + Rng.Int(mDepth / 10))));
  ...
  Rect createRectRoom(Connector connector, int width, int height) {
    ...
    if (connector == null) {
      ...
      x = rng.triangleInt((creep.level.width - width) ~/ 2,
                          (creep.level.width - width) ~/ 2 - 4);
      y = rng.triangleInt((creep.level.height - height) ~/ 2,
                          (creep.level.height - height) ~/ 2 - 4);
    ...
    if (!creep.isOpen(bounds.inflate(1),
  ...
  void addRoomEdgeConnectors(Connector connector, Rect edge, Direction dir) {
    ...
    for (final pos in edge) {
      ...
      if (!skip && (rng.range(100) < creep.options.chanceOfRoomConnector)) {
        creep.addRoomConnector(pos, dir);
        skip = true;
      } else {
        skip = false;
      }
  ...
  private void PlaceDoor(Vec pos)
  {
    ...
    if (choice < creep.Options.ChanceOfOpenDoor)
    {
      creep.SetTile(pos, TileType.DoorOpen);
    }
    else if (choice - creep.Options.ChanceOfOpenDoor < creep.Options.ChanceOfClosedDoor)
    {
      creep.SetTile(pos, TileType.DoorClosed);
    }
    else
    {
      creep.SetTile(pos, TileType.Floor);
  ...
  private void Populate(Rect bounds, int monsterDensity, int itemDensity, int depth)
  {
    ...
    foreach (Vec pos in bounds)
    {
      creep.Populate(pos, monsterDensity, itemDensity, depth);
    }
  }
  ...
  public class RoomDecorator : IRoomDecorator
  {
    ...
    {
      mFactory.creep.SetTile(pos, mDecoration);
    }
