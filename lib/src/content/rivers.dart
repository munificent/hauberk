...
/// Mixin for [Dungeon2] that adds support for rivers.
abstract class Rivers implements DungeonBase {
  void _displace(_RiverPoint start, _RiverPoint end, Set<Vec> path) {
    var h = start.x - end.x;
    var v = start.y - end.y;
    ...
    if (length > 1.0) {
      ...
      for (var y = y1; y <= y2; y++) {
        for (var x = x1; x <= x2; x++) {
          ...
          } else if (lengthSquared <= shoreSquared && isUnused(x, y)) {
  ...
  Iterable<String> addRiver() sync* {
    // Midpoint displacement.
    // Consider also squig curves from: http://algorithmicbotany.org/papers/mountains.gi93.pdf.
    yield "Carving river";
    var start =
        new _RiverPoint(rng.float(width.toDouble()), -4.0, rng.float(1.0, 3.0));
    var end = new _RiverPoint(
        rng.float(width.toDouble()), height + 4.0, rng.float(1.0, 3.0));
    var mid = new _RiverPoint(rng.float(width * 0.25, width * 0.75),
    ...
    for (var pos in path) {
      ...
      for (var x = pos.x; x >= 0; x--) {
        if (getTile(x, pos.y) == Tiles.grass) {
          westShore = x + 1;
          break;
        }
      }
      ...
      for (var y = pos.y; y >= 0; y--) {
        if (getTile(pos.x, y) == Tiles.grass) {
          northShore = y + 1;
          break;
        }
      }
    ...
    if (bridges.isNotEmpty) {
      ...
      for (var i = 0; i < count; i++) {
        ...
        for (var pos in shortest) {
          setTile(pos.x, pos.y, Tiles.bridge, TileState.natural);
        }
      }
    }
